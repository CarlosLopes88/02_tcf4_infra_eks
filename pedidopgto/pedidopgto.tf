...
data "aws_security_group" "eks" {
  filter {
    name   = "group-name"
    values = ["eks-sg-pedidopgto"]  # Atualizar para o nome específico
  }
...
module "eks" {
  ...
  eks_managed_node_groups = {
    pedidopgto-node-group = {  # Mais consistente com nomenclatura
      desired_size = 1
      min_size     = 1
      max_size     = 2
    ...
    template {
      ...
      spec {
        container {
          ...
          # Adicionando as variáveis de ambiente para dependências
          env {
          ...
          # Adicionando configuração para conectar ao banco de dados
          env {
            name  = "DB_USERNAME"
